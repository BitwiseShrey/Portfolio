...
export default function LightboxModal({ isOpen, onClose, item, event }) {
  const currentItem = item || event;
  const isModalOpen = isOpen !== undefined ? isOpen : Boolean(currentItem);
  ...
  const gallery = currentItem?.gallery && currentItem.gallery.length > 0
    ? currentItem.gallery
    : currentItem?.image
      ? [currentItem.image]
      : [];
  ...
  }, [isModalOpen, onClose, gallery.length]);

  if (!isModalOpen || !currentItem) return null;

  const currentMediaSrc = gallery[activeImgIdx] || currentItem.image || currentItem.poster;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 md:p-10 bg-black/85 backdrop-blur-xl animate-fade-in">
      ...
      <div className="relative w-full max-w-4xl max-h-[92vh] overflow-y-auto rounded-3xl bg-[#0f0e16] border border-purple-500/40 shadow-2xl z-10 flex flex-col">
        ...
        <div className="p-6 sm:p-8 space-y-5">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-xs font-mono uppercase px-2.5 py-1 rounded-md bg-purple-500/20 text-purple-300 font-semibold border border-purple-500/30">
            ...
            <div className="pt-4 border-t border-white/10">
              ...
              <div className="flex flex-wrap gap-2">
                {currentItem.highlights.map((h, i) => (
                  <span
                    key={i}
                    className="text-xs px-3 py-1 rounded-lg bg-white/[0.04] border border-white/[0.08] text-zinc-300 font-medium"
                  >
                    &bull; {h}
                  </span>
                ))}
