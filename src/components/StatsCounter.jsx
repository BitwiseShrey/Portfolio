...
export default function StatsCounter() {
  ...
  return (
    <section id="impact" ref={ref} className="py-16 sm:py-20 border-y border-[#DFB6B2]/10 bg-[#190019] relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {IMPACT_METRICS.map((metric, idx) => (
            <div
              key={idx}
              data-spotlight
              className="rounded-2xl sm:rounded-3xl bg-[#2B124C] border border-[#DFB6B2]/12 hover:border-[#854F6C]/60 p-6 sm:p-7 flex flex-col justify-between transition-all duration-500 hover:-translate-y-1.5 hover:shadow-2xl hover:shadow-black/70 shadow-lg group relative overflow-hidden"
              style={{
                opacity: inView ? 1 : 0,
            ...
            >
              ...
              <div 
                ...
                style={{
                  background: 'radial-gradient(circle 180px at var(--mouse-x, 50%) var(--mouse-y, 50%), rgba(223, 182, 178, 0.12), transparent 70%)'
                }}
              />

              {/* Restrained Accent Line on Top */}
              <div className="w-8 h-[2px] bg-[#854F6C]/40 group-hover:w-14 group-hover:bg-[#DFB6B2] transition-all duration-500 mb-6" />

              <div>
                <div className="flex items-center justify-between">
                  <div className="text-4xl sm:text-5xl lg:text-6xl font-bold text-[#FBE4D8] font-mono tracking-tight transition-colors">
                    <AnimatedNumber value={metric.value} shouldAnimate={inView} delay={idx * 110} />
                  </div>
                  <span className="w-1.5 h-1.5 rounded-full bg-[#854F6C] group-hover:bg-[#DFB6B2] transition-all"></span>
                </div>

                <h3 
                  className="text-sm sm:text-base font-bold text-[#FBE4D8]/90 font-display mt-3 leading-snug transition-all duration-700"
                  style={{
                    opacity: inView ? 1 : 0,
              ...
              <div 
                className="pt-4 mt-4 border-t border-[#DFB6B2]/10 text-xs text-[#DFB6B2]/80 font-mono transition-all duration-700"
                style={{
                  opacity: inView ? 1 : 0,
