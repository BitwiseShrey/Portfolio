...
export default function EventShowcase() {
  ...
  const workshopEvent = FEATURED_EVENTS.find(e => e.id === 'aws-workshops') || FEATURED_EVENTS[3] || FEATURED_EVENTS[0];
  const neonBadminton = FEATURED_EVENTS.find(e => e.id === 'neon-badminton-2026') || FEATURED_EVENTS[2] || FEATURED_EVENTS[0];

  const eventList = [
    {
      event: healthHack,
      badge: "Lead Coordinator · Feb 2026",
      title: healthHack?.title || "Health Hackathon 2026",
      caption: healthHack?.caption || "Delivering the opening address at the university podium in partnership with Johns Hopkins Whiting School."
    },
    {
      event: awsEvent,
      badge: "Chapter Leader · 500+ Attendees",
      title: "AWS Student Community Assembly",
      caption: awsEvent?.caption || "Convening the campus cloud community and core team in the main auditorium with guest AWS mentors."
    },
    {
      event: neonBadminton,
      badge: "Co-Organizer · Advitya 2026",
      title: neonBadminton?.title || "Neon Badminton — Advitya 2026",
      caption: neonBadminton?.caption || "Curating a high-intensity UV neon tournament in collaboration with the Startup Club."
    },
    {
      event: workshopEvent,
      badge: "Lead Instructor · Hands-on Labs",
      title: workshopEvent?.title || "AWS Technical Workshops",
      caption: workshopEvent?.caption || "Conducting practical cloud computing and serverless architecture lab sessions."
    }
  ];
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              {MORE_EXPERIENCES.map((exp, idx) => (
                ...
                >
                  ...
                  <h4 className="text-sm font-bold font-display text-white">{exp.title}</h4>
